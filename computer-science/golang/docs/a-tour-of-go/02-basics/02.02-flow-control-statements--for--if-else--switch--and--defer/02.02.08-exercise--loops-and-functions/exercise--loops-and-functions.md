...
As a way to play with function sand loops, let's implement a square root function: given a number `x`, we want to find the number `z` for which `z^2` is most nearly `x`.
